...
public class PdfBuilderTests {
	...
	// ============================================================
	//  Round 4 tests: content-hash incremental skip + -file-line-error
	// ============================================================

	/// <summary>首次构建写入 sidecar；源/配置/模板未变时第二次跑应跳过 xelatex。</summary>
	[Fact]
	public void Build_IncrementalEnabled_NoChange_SkipsXelatex() {
		var (tmpDir, builder, runner) = CreateBuilderFixtureWithFakeRunner(
			"""{ "PROGRAM": { "build": { "incremental": true } } }""");
		var outputPdfPath = Path.Combine(tmpDir, "out.pdf");
		try {
			// 第一次：PDF 不存在 → 走全量
			builder.Build();
	...
	[Fact]
	public void Build_IncrementalEnabled_SourceChanged_Rebuilds() {
		var (tmpDir, builder, runner) = CreateBuilderFixtureWithFakeRunner(
			"""{ "PROGRAM": { "build": { "incremental": true } } }""");
		var outputPdfPath = Path.Combine(tmpDir, "out.pdf");
		try {
			...
			File.WriteAllText(outputPdfPath, "%PDF-1.4 stub");
			// 改 source 文件
			File.WriteAllText(Path.Combine(tmpDir, "src", "a.cpp"), "int main() { return 42; }");
			runner.Calls.Clear();
			builder.Build();
			Assert.Equal(2, runner.Calls.Count);
		} finally {
	...
	[Fact]
	public void Build_IncrementalEnabled_ConfigChanged_Rebuilds() {
		var (tmpDir, _, runner) = CreateBuilderFixtureWithFakeRunner(
			"""{ "PROGRAM": { "build": { "incremental": true } }, "TEX": { "title": { "content": "V1" } } }""");
		var outputPdfPath = Path.Combine(tmpDir, "out.pdf");
		try {
			var (tmpDir2, builder, runner2) = CreateBuilderFixtureWithFakeRunner(
				"""{ "PROGRAM": { "build": { "incremental": true } }, "TEX": { "title": { "content": "V1" } } }""");
			builder.Build();
			File.WriteAllText(outputPdfPath, "%PDF-1.4 stub");
			// 改 config（覆盖 title）
			...
			File.WriteAllText(cfgPath,
				"""{ "PROGRAM": { "build": { "incremental": true } }, "TEX": { "title": { "content": "V2" } } }""");
			var logger = new TestLogger();
			var texParser = new ConfigParser("TEX", logger);
			var programParser = new ConfigParser("PROGRAM", logger);
			texParser.ParseConfigFile(File.ReadAllText(cfgPath), cfgPath);
			programParser.ParseConfigFile(File.ReadAllText(cfgPath), cfgPath);
			var options = new BuildSubcommandOptions(
				new DirectoryInfo(Path.Combine(tmpDir, "src")),
				new FileInfo(outputPdfPath),
				new FileInfo(cfgPath),
				Verbose: false,
				TemplateDir: null);
			var resMgr = new ManifestResourceManager(logger);
			var builder2 = new PdfBuilder(logger, options, texParser, programParser, resMgr, runner2);
			runner2.Calls.Clear();
			builder2.Build();
			Assert.Equal(2, runner2.Calls.Count);
			Directory.Delete(tmpDir2, recursive: true);
		} finally {
			Directory.Delete(tmpDir, recursive: true);
		}
	}
	...
	[Fact]
	public void Build_IncrementalEnabled_TemplateChanged_Rebuilds() {
		var (tmpDir, builder, runner) = CreateBuilderFixtureWithFakeRunner(
			"""{ "PROGRAM": { "build": { "incremental": true } } }""");
		var outputPdfPath = Path.Combine(tmpDir, "out.pdf");
		var templateDir = Path.Combine(tmpDir, "templates");
		try {
			builder.Build();
			File.WriteAllText(outputPdfPath, "%PDF-1.4 stub");
			// 创建外部 TemplateDir 并改 Main.tex
			Directory.CreateDirectory(templateDir);
			File.WriteAllText(Path.Combine(templateDir, "Main.tex"),
				"\\documentclass{article}\\begin{document}<<CONTENT>>\\end{document}");
			// 重建 builder，使用新 TemplateDir
			var logger = new TestLogger();
			var texParser = new ConfigParser("TEX", logger);
			var programParser = new ConfigParser("PROGRAM", logger);
			var cfgPath = Path.Combine(tmpDir, "cfg.json");
			...
			var options = new BuildSubcommandOptions(
				...
				new FileInfo(cfgPath),
				Verbose: false,
				TemplateDir: new DirectoryInfo(templateDir));
			var resMgr = new ManifestResourceManager(logger);
			var builder2 = new PdfBuilder(logger, options, texParser, programParser, resMgr, runner);
			runner.Calls.Clear();
			builder2.Build();
			Assert.Equal(2, runner.Calls.Count);
		} finally {
	...
	[Fact]
	public void Build_IncrementalDisabled_Default_DoesNotReadSidecar() {
		// incremental 默认 false → 即使有 sidecar + PDF，也走全量构建
		var (tmpDir, builder, runner) = CreateBuilderFixtureWithFakeRunner("{}");
		var outputPdfPath = Path.Combine(tmpDir, "out.pdf");
		try {
			// 主动写一个空 sidecar + 一个 PDF，模拟"上次已成功"
			File.WriteAllText(outputPdfPath, "%PDF-1.4 stub");
			var sidecarPath = Path.ChangeExtension(outputPdfPath, ".tbuild");
			File.WriteAllText(sidecarPath, "anyhash");
			builder.Build();
			Assert.Equal(2, runner.Calls.Count);
		} finally {
			Directory.Delete(tmpDir, recursive: true);
	...
	[Fact]
	public void Build_IncrementalEnabled_PdfMissing_Rebuilds() {
		// PDF 缺失 → 即使有 sidecar 也走全量构建
		var (tmpDir, builder, runner) = CreateBuilderFixtureWithFakeRunner(
			"""{ "PROGRAM": { "build": { "incremental": true } } }""");
		try {
			// 首次构建写 sidecar（PDF 不存在）
			builder.Build();
			var sidecarPath = Path.Combine(tmpDir, "out.tbuild");
	...
	[Fact]
	public void Build_IncrementalEnabled_XelatexFails_LeavesPriorSidecarIntact() {
		// 验证 sidecar 在失败路径不被覆写
		var (tmpDir, builder, runner) = CreateBuilderFixtureWithFakeRunner(
			"""{ "PROGRAM": { "build": { "incremental": true } } }""");
		var outputPdfPath = Path.Combine(tmpDir, "out.pdf");
		var sidecarPath = Path.Combine(tmpDir, "out.tbuild");
		try {
			...
			builder.Build();
			Assert.True(File.Exists(sidecarPath));
			var hash1 = File.ReadAllText(sidecarPath);
			File.WriteAllText(outputPdfPath, "%PDF-1.4 stub");

			// 2) 失败路径：先 touch 源文件让指纹失配（否则 incremental skip 会先于 xelatex 失败触发）；
			//    删 PDF 让 xelatex 非零退出走 XelatexFailure 分支（PDF 存在时只 warn 不 fail）；
			//    再让两次 xelatex pass 都返回非零。
			File.WriteAllText(Path.Combine(tmpDir, "src", "a.cpp"), "int main() { return 99; }");
			File.Delete(outputPdfPath);
			runner.Results.Clear();
			runner.Results.Enqueue(new XelatexResult(1, "boom", false));
			runner.Results.Enqueue(new XelatexResult(1, "boom", false));
			var exitCode = builder.Build();
			Assert.Equal(ExitCodes.XelatexFailure, exitCode);
			// sidecar 应保持不变（失败路径不写）
			Assert.Equal(hash1, File.ReadAllText(sidecarPath));
		} finally {
			Directory.Delete(tmpDir, recursive: true);
		}
	}
	...
	// ============================================================
	//  Round 4 tests: TEX.code.minted_outputdir override (CI cache reuse)
	// ============================================================
	...
	public void GenerateTexContent_MintedOutputdirEmpty_DefaultsToPdfDir() {
		...
		try {
			var tex = builder.GenerateTexContent_ForTest();
			var expectedDir = Path.Combine(tmpDir, "out.pdf");
	...
	// ============================================================
	//  Round 4 tests: ReplaceMainPlaceholders 单遍 regex 重写
	// ============================================================

	/// <summary>
	/// 回归测试：Round 3→4 把 O(K·N) 的 StringBuilder.Replace 循环改为单遍 regex，
	/// 默认 config 下的 TeX 输出应与原实现完全一致。用 hash 避免逐行比对长字符串。
	/// </summary>
	[Fact]
